...
import { AssignSeatsInput } from '../event/models/inputs/assign-seat.input.js';
import { SeatWriteService } from '../event/services/seat-write.service.js';
...
export class TicketHandler implements KafkaEventHandler {
  ...
  async handle(
    ...
    switch (topic) {
      case getTopic('addGuest'):
        await this.addGuestId(data as { payload: AssignSeatsInput });
  ...
  private async addGuestId(data: { payload: AssignSeatsInput }): Promise<void> {
